...
let startsWith = (string: string, search: string) => {
...
let pick = (routes: Array<{ path: string }>, uri: string) => {
  let match;
  let default_;
...
let insertParams = (path: string, params: Array<string>) => {
  let segments = segmentize(path);
  return (
    "/" +
    segments
      .map((segment) => {
        let match: any = paramRe.exec(segment); // TODO:@mike fix type
...
let rankRoute = (route: any, index: number) => {
...
let rankRoutes = (routes: Array<{ path: string }>) =>
  routes
    .map(rankRoute)
    .sort((a, b) =>
...
let segmentize = (uri: string): Array<string> =>
  uri
    // strip starting/ending slashes
    .replace(/(^\/+|\/+$)/g, "")
    .split("/");

let addQuery = (pathname: string, query: string) =>
